...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        static List<Show> BestDrama(List<Show> shows)
        {
            return shows
                .OrderByDescending(s => s.ImdbRating)
        ...
        static List<Show> FirstLongRunningTopRated(List<Show> shows)
        {
            return shows.OrderBy(s => (s.EndYear - s.StartYear) > 10).Where(s => s.ImdbRating < 8).OrderBy(s => s.Name).Take(1).ToList();
        ...
        {
            return shows.FirstOrDefault(s => s.Name.Split(" ").Count() == shows.Max(s => s.Name.Split(" ").Count()));
        }
